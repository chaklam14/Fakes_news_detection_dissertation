...
df = readtable('gossip_combine.csv');
%%
...
B = TreeBagger(100,X,Y,'OOBPrediction','On','OOBPredictorImportance','on');
...
MCC3 = [];
for i = depths
    B3 = TreeBagger(70,X,Y,'MaxNumSplits',i);
...
% depth=95 is the best

%%
%%%% max depth of 95 and 70 trees
B4 = TreeBagger(70,X,Y,'OOBPrediction','On','MaxNumSplits',95);
%% label
...
%% 80 depths and 38 trees
B5 = TreeBagger(60,X,Y,'OOBPrediction','On','MaxNumSplits',80);

%% label
B5_label = predict(B5,test);
testy = str2double(B5_label);

cm5 = confusionchart(actualy,testy);
con_mat5 = confusionmat(actualy,testy);

TP = con_mat5(4);
TN = con_mat5(1);
FN = con_mat5(3);
FP = con_mat5(2);
accuracy5 = (TP+TN)/(TP+TN+FN+FP);
MCC5 = (TP*TN-FP*FN)/(sqrt((TP+FP)*(TP+FN)*(TN+FP)*(TN+FN)));

%%
figure;
oobErrorBaggedEnsemble = oobError(B5);
plot(oobErrorBaggedEnsemble)
xlabel 'Number of grown trees';
ylabel 'Out-of-bag classification error';
